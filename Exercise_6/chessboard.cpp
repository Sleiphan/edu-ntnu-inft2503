...
public:
  ...
  std::function<void(const Piece &piece, const std::string &from, const std::string &to)> post_move_cb;
  std::function<void(const Piece &piece, const std::string &from, const std::string &to)> cannot_move_cb;
  std::function<void(const Piece &piece, const std::string &from)> piece_removed_cb;
  std::function<void(const std::string &from)> piece_not_found_cb;
  std::function<void(const std::string &faction)> lost_game_cb;
    ...
    if (piece_from)
    {
      if (piece_from->valid_move(from_x, from_y, to_x, to_y))
      {
        ...
        if (piece_to)
        {
          ...
          {
            piece_removed_cb(*piece_to.get(), to);
            if (auto king = dynamic_cast<King *>(piece_to.get()))
              lost_game_cb(king->color_string());
          }
          else
          {
            // piece in the from square has the same color as the piece in the to square
            cannot_move_cb(*piece_from.get(), from, to);
            return false;
          }
        }
        piece_to = move(piece_from);
        post_move_cb(*squares[to_x][to_y].get(), from, to);
        return true;
      }
      else
      {
